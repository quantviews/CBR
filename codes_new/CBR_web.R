...
# Функция DepoDynamicXML http://cbr.ru/DailyInfoWebServ/DailyInfo.asmx?op=DepoDynamicXML
...
#Итоги аукциона прямого РЕПО (как XMLDocument)
# <xs:element name="Dt" msdata:Caption="Дата" type="xs:dateTime"/>
#   <xs:element name="avg_deal" msdata:Caption="Общий объем заключенных сделок, млн." type="xs:decimal" minOccurs="0"/>
#   <xs:element name="avg_yield" msdata:Caption="Средневзвешенная ставка, % годовых" type="xs:decimal" minOccurs="0"/>
#   <xs:element name="snd_date" msdata:Caption="Дата исполнения второй части РЕПО" type="xs:dateTime" minOccurs="0"/>
#   <xs:element name="day_repo" msdata:Caption="Срок РЕПО, дни" type="xs:decimal" minOccurs="0"/>
#   <xs:element name="mTime" msdata:Caption="время аукциона" type="xs:string"/>
#   <xs:element name="PCode" msdata:Caption="Код расчетов по сделкам" type="xs:string" minOccurs="0"/>
#   <xs:element name="bid" msdata:Caption="Объем спроса, млн. руб." type="xs:decimal" minOccurs="0"/>
#   <xs:element name="max_state_intrate" msdata:Caption="Максимальная заявленная ставка, % годовых" type="xs:decimal" minOccurs="0"/>
#   <xs:element name="min_state_intrate" msdata:Caption="Минимальная заявленная ставка, % годовых" type="xs:decimal" minOccurs="0"/>
#   <xs:element name="cut_off_rate" msdata:Caption="Ставка отсечения, % годовых" type="xs:decimal" minOccurs="0"/>
#   <xs:element name="avg_deal_lim" msdata:Caption="Объем заключенных сделок в рамках лимита, млн. руб." type="xs:decimal" minOccurs="0"/>
#   <xs:element name="avg_yield_lim" msdata:Caption="Средневзвешенная ставка по заявкам, удовлетворенным в рамках лимита, % годовых" type="xs:decimal" minOccurs="0"/>
#   <xs:element name="first_date" msdata:Caption="Дата исполнения первой части сделки РЕПО" type="xs:dateTime" minOccurs="0"/>
#   </xs:sequence>
...
REPOXML <- function(DateFrom, DateTo){
  ...
  df[, 'snd_date']<- as.Date(as.POSIXct(df[, 'snd_date']))+1
  df[, 'first_date']<- as.Date(as.POSIXct(df[, 'first_date']))+1
  num <- c('avg_deal', 'avg_yield', 'bid', 'max_state_intrate', 'min_state_intrate',  "cut_off_rate",
           "avg_deal_lim", "avg_yield_lim")
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  a <- cleanup[[1]]
  for (j in 1:length(cleanup)){ #по всем строкам
    ...
    for (i in 1:length(vars)){
      ...
      if (length(drow) >0) df[j,i] <- drow else df[j,i] = NA #если нет каких-то значений, то поставить NA
    }
    #print(j)    
  }
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
   ...
   df <- as.data.frame(matrix(nrow =length(cleanup), ncol = length(vars), dimnames = list(NULL, vars)))
   # пройтись по всем переменным в цикле (вложенный цикл необходим так в некоторых элементах есть не все ячейки)
   a <- cleanup[[1]]
   for (j in 1:length(cleanup)){ #по всем строкам
      a <- cleanup[[j]] # выделить отдельную ячейку и пройтись по ней
      for (i in 1:length(vars)){
   ...
   return(df)
   #Тип выплаты: RD - погашение;  WT - купонная выплата, PP - частичное погашение  
}
...
OstatDynamicXML <- function(fromDate, ToDate){
  ...
  df[,2:3]<- apply(df[,2:3], 2, as.numeric)
  df <- xts((df[,-1]), order.by = df[,1])
  return(df)
}
...
#Фиксированная процентная ставка по ломбардным кредитам Банка России сроком на один календарный день (как XMLDocument)
...
#Фиксированные процентные ставки по кредитам, обеспеченным нерыночными активами или поручительствами (как XMLDocument)
