...
chart.StackedBar(repo.volume/1000, las =1, colorset = ief.palette, border = NA, minor.ticks = FALSE,
                 ylab = 'bln RUB')
...
buffer <- na.locf(merge.xts(Ruonia$ruo, Depo$TomNext, Repo$Rate7Day, mosprime$T3M, repo.ir))
...
chart.TimeSeries.IEF(buffer, date.format = '%b-%Y', main = '',                      
                     colorset = ief.palette, legend.loc = "topleft", ylab = '%', xlab = '')
...
gg <- gg['2012::']
Plot2.IEF(saldo, Ruonia$ruo, name1= 'Liquidity saldo', name2 = 'RUONIA', ylab1='bln RUB')
